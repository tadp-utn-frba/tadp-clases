...
package object objetos {
  val todasLasMaterias: List[Materia] = List(
    ...
    Materia(nombre = "Paradigmas de Programación", electiva = false, cursos = List(
      Curso(codigo = 2004, titular = Docente(apellido = "Aimar", email = null), inscriptos = List(
  ...
  val todasLasMateriasLazy: Seq[Materia] = todasLasMaterias.to(LazyList)

  def enviarEmailA(mail: String): String | Null = if (mail.contains("@")) mail else null
}
